...
- `PRICE_ENGINE_RECONCILIATION_SCHEMA_PLAN_V1.md` defines the proposed PostgreSQL / Supabase table model for CoreLogic + MLS + field + Dropbox + ZipForms + manual property imports, canonical property storage, field-level reconciliation, repeatable MLS child records such as rooms and media, evidence/document versioning, checklist completeness, client file visibility, overrides, and freshness tracking before any Price Engine UI dependence on DB-backed property authority.
