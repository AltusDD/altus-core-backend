...
FIXTURE_ROOT = ROOT / "docs" / "contracts" / "fixtures" / "title_rate_quote"
PROVIDER_ENV = "PRICE_ENGINE_TITLE_RATE_PROVIDER"
...
from title_rate_handler import handle_title_rate_quote  # noqa: E402
...
class TitleRateQuoteContractTests(unittest.TestCase):
    def setUp(self) -> None:
        self.original_provider = os.environ.get(PROVIDER_ENV)

    def tearDown(self) -> None:
        if self.original_provider is None:
            os.environ.pop(PROVIDER_ENV, None)
        else:
            os.environ[PROVIDER_ENV] = self.original_provider

    def test_stub_success_contract_matches_fixture(self) -> None:
        os.environ[PROVIDER_ENV] = "stub"

        response = handle_title_rate_quote(
            FakeRequest(load_fixture("stub_request.json")),
        ...
        self.assertEqual(
            ...
            load_fixture("stub_response.json"),
        )

    def test_missing_provider_contract_matches_fixture(self) -> None:
        os.environ.pop(PROVIDER_ENV, None)

        response = handle_title_rate_quote(
            FakeRequest(load_fixture("error_no_provider_request.json")),
            lambda: {"x-altus-build-sha": "test-sha"},
        )
        ...
        self.assertEqual(
            json.loads(response.get_body().decode("utf-8")),
            load_fixture("error_no_provider_response.json"),
        )
